...
module Jekyll
        ...
        def render(context)
            if context['page']['mapping']
                ...
                if @engine == 'google_static'
                    ...
                    return "<div id=\"jekyll-mapping\" class=\"col-12\" style=\"height:#{@height}px;\"></div>"
                end
            end
        end
    end
    ...
    class MapIndexTag < Liquid::Tag
        ...
        def render(context)
            ...
            for post in posts
                if post.data.has_key?('mapping') && post.data['mapping'].has_key?('latitude') && post.data['mapping'].has_key?('longitude')
                    ...
                    if Jekyll.configuration({})['baseurl']
                        postinfo['link'] = "#{Jekyll.configuration({})['baseurl']}#{post.url}"
                    else
            ...
            if (@engine == 'google_js')
                return "
                    <div id='jekyll-mapping' class='col-12'  style='height:#{@height}px;'>
                    </div>
